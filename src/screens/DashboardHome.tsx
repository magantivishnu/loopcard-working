...
// ⚠️ Set this to your LAN IP or production domain
const WEB_BASE = "http://192.168.1.42:3000";
...
export default function DashboardHome({ navigation }: any) {
  ...
  // ✅ Open preview inside the app (WebView)
  const openPreviewInApp = (slug?: string | null) => {
    if (!slug) {
      Alert.alert("No slug", "This card does not have a public slug yet.");
      return;
    }
  ...
  const downloadQR = async (slug?: string | null) => {
    ...
    const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=1024x1024&data=${encodeURIComponent(
      targetUrl
    )}`;

    try {
      const filename = `loopcard-qr-${slug}.png`;
      const localUri = `/data/user/0/host.exp.exponent/cache/${filename}`;
      const { uri } = await downloadAsync(qrUrl, localUri);

      if (await Sharing.isAvailableAsync()) {
        await Sharing.shareAsync(uri, { mimeType: "image/png", dialogTitle: "Share QR code" });
      } else {
        Alert.alert("Downloaded", `QR saved temporarily:\n${uri}`);
      }
    } catch (e) {
    ...
    return (
      ...
      >
        ...
        <View style={{ flexDirection: "row", alignItems: "center", marginBottom: 12 }}>
          ...
          >
            {c.avatar_url ? (
              <Image source={{ uri: c.avatar_url }} style={{ width: 52, height: 52 }} resizeMode="cover" />
            ) : (
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: "#F7F7F7" }} contentContainerStyle={{ padding: 16 }}>
      ...
      <Text style={{ color: "#666", marginBottom: 16 }}>
        Preview (in-app), share, or save the QR for any of your cards.
      </Text>
